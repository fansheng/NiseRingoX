...
-(void)showWindow:(id)sender{
    ...
    // 起動処理
    self.booting_check_oldest_type.integerValue = 0;
    if ([defaults objectForKey:@"booting.converter.inverse_type"]==nil) {
        self.booting_check_inverse_type.integerValue = 1;
    }
    else{
        self.booting_check_inverse_type.integerValue = [defaults integerForKey:@"booting.converter.inverse_type"];
    }
    if ([defaults objectForKey:@"booting.converter.thumbnail"]==nil) {
        self.booting_check_ghost_thumbnail.integerValue = 1;
    }
    else{
        self.booting_check_ghost_thumbnail.integerValue = [defaults integerForKey:@"booting.converter.thumbnail"];
    }
    if ([defaults objectForKey:@"booting.converter.zeropoint_balloon"]==nil) {
        self.booting_check_zeropoint_balloon.integerValue = 1;
    }
    else{
        self.booting_check_zeropoint_balloon.integerValue = [defaults integerForKey:@"booting.converter.zeropoint_balloon"];
    }
    
    // メールチェック
    workstr=[defaults stringForKey:@"mailcheck_server"];
    self.mailcheck_fld_server.stringValue = (workstr==nil ? @"" : workstr);
    workstr=[defaults stringForKey:@"mailcheck_username"];
    self.mailcheck_fld_username.stringValue = (workstr==nil ? @"" : workstr);
    workstr=[defaults stringForKey:@"mailcheck_password"];
    self.mailcheck_fld_password.stringValue = (workstr==nil ? @"" : workstr);
    
    self.mailcheck_check_enable.integerValue = [defaults integerForKey:@"mailcheck_check_enable"];
    self.mailcheck_check_autocheck.integerValue = [defaults integerForKey:@"mailcheck_check_autocheck"];
    self.mailcheck_fld_autocheck_interval.integerValue = [defaults integerForKey:@"mailcheck_autocheck_interval"];
    
    // SSTP
    // エントリが存在しなければ0が返る→offの時も0が返る→デフォルトでonの方がいいと思うけど、面倒だから諦めようか。
    self.sstp_switch.integerValue = [defaults integerForKey:@"sstp_switch"];
    ...
    self.display_sb_activate_on_script.integerValue = [defaults integerForKey:@"display.activate_on_script"];
    ...
    self.display_sb_fontnamefield.stringValue = fontname;
    self.display_sb_fontsizefield.floatValue = fontsize;
    NSFontManager* fm = [NSFontManager sharedFontManager];
    [fm setSelectedFont:[NSFont fontWithName:fontname size:fontsize] isMultiple:NO];
    fm.action = @selector(_font_changed);
    double display_transparency_val = [defaults doubleForKey:@"display_slider_transparency"]; // 未定義なら0。
    self.display_sb_slider_transparency.doubleValue = display_transparency_val;
    ...
    self.display_sb_slider_waitrate.doubleValue = display_waitrate_val;
    
    if ([defaults objectForKey:@"display.balloon.fadeout"] == nil) {
        self.display_sb_balloon_fadeout.integerValue = 1; // デフォルトでon。
    }
    else {
        self.display_sb_balloon_fadeout.integerValue = [defaults integerForKey:@"display.balloon.fadeout"];
    }
    NSInteger ct = [defaults integerForKey:@"display.balloon.clickthrough"];
    //SCFoundation.BALLOON_USES_CLICK_THROUGH = (ct != 0);
    self.display_sb_balloon_clickthrough.integerValue = ct;
    //   ゴーストマネージャ
    ...
    self.display_gm_preview_fpath.stringValue = bgimage_path;
	
    // その他
    self.misc_always_show_vanish.integerValue = [defaults integerForKey:@"misc.always_show_vanish"];
    self.misc_delete_after_online_install.integerValue = [defaults integerForKey:@"misc.delete_after_online_install"];
    self.misc_load_whole_surfaces.integerValue = [defaults integerForKey:@"misc.load_whole_surfaces_on_booting_surface_server"];
    self.misc_disable_seriko_animation.integerValue = [defaults integerForKey:@"misc.disable_seriko_animation"];
    self.misc_show_dev_interfaces.integerValue = [defaults integerForKey:@"misc.show_dev_interfaces"];
    self.misc_lightmode_on_sstp.integerValue = [defaults integerForKey:@"misc.lightmode_on_sstp"];
    self.misc_reset_surface_on_sstp.integerValue = [defaults integerForKey:@"misc.reset_surface_on_sstp"];
...
-(void)_font_changed{
    ...
    self.display_sb_fontnamefield.stringValue = name;
    self.display_sb_fontsizefield.floatValue = size;
